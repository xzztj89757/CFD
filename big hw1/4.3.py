...
def upwind_3d(u):
    ...
    for i in range(Nx):
        ...
        ux[i] = (2 * u[ip1] + 3 * u[i] - 6 * u[i - 1] + u[i - 2]) / (6 * dx)
    return ux
...
colors = {
    '1d': '#E24A33',  # Orange-Red
    '2d': '#348ABD',  # Blue
    '3d': '#FBC15E',  # Golden Yellow
}
markers = {
    '1d': 's',
    '2d': 'o',
...
for N in grid_sizes:
    ...
    u_1d, err_1d = run_simulation(u0, Runge_Kutta4, upwind_1d)
    u_2d, err_2d = run_simulation(u0, Runge_Kutta4, upwind_2d)
    u_3d, err_3d = run_simulation(u0, Runge_Kutta4, upwind_3d)

    errors['1d'].append(err_1d)
    errors['2d'].append(err_2d)
    errors['3d'].append(err_3d)

    # Create a figure for plotting

    fig, ax = plt.subplots()
    ax.scatter(x, u_1d, color=colors['1d'], label='upwind_1d', s=12, marker=markers['1d'])
    ax.scatter(x, u_2d, color=colors['2d'], label='upwind_2d', s=12, marker=markers['2d'])
    ax.scatter(x, u_3d, color=colors['3d'], label='upwind_3d', s=12, marker=markers['3d'])
